...
#####################################################################################
def get_data_list(images_path, labels_path, contours_path, image_ext):
  ...
  return images_list, labels_list, contours_list
...
def read_data(filename_queue):
  ...
  if len(filename_queue)==3:
    contour.set_shape([FLAGS.train_image_source_size,FLAGS.train_image_source_size,3])
  else:
    contour  = None
...
#####################################################################################
def process_image_and_label(image, label, contour, source_size, target_size, 
  ...
  if image.dtype != tf.float32:
    ...
    if contour  is not None:
      contour  = tf.cast(contour ,tf.float32)

  # Randomly crop the image
  if source_size >= target_size:
    slice_index = int((target_size-ground_truth_size)/2)
    concat = tf.concat([image, label, contour ], axis=2)
    crop = tf.random_crop(concat, [target_size,target_size,9])
    ...
    contour  = tf.slice(crop,[slice_index,slice_index,6],[ground_truth_size,ground_truth_size,2])
  else:
    ...
    contour  = tf.expand_dims(contour, 0)
    contour  = tf.image.resize_bilinear(contour, [ground_truth_size, ground_truth_size], align_corners=False)
    contour  = tf.squeeze(contour , [0])
  ...
  if FLAGS.random_rotation:
    ...
    if contour  is not None:
      contour  = tf.image.rot90(contour , k=num_rotation[0])
  ...
  if FLAGS.random_flipping:
    ...
    label = control_flow_ops.cond(pred=tf.equal(random_var, 0),
      ...
      fn2=lambda: label)
    if contour  is not None:
      contour  = control_flow_ops.cond(pred=tf.equal(random_var, 0),
        fn1=lambda: tf.image.flip_left_right(contour ),
        fn2=lambda: contour )
    ...
    label = control_flow_ops.cond(pred=tf.equal(random_var2, 0),
      ...
      fn2=lambda: label)
    if contour  is not None:
      contour  = control_flow_ops.cond(pred=tf.equal(random_var2, 0),
        fn1=lambda: tf.image.flip_up_down(contour ),
        fn2=lambda: contour )
  ...
  return distorted_image, label, contour 
...
#####################################################################################
def generate_image_label_contour_batch(image, label, contour , min_queue_examples, batch_size, shuffle):
  num_preprocess_threads = 16
  if shuffle:
    images, labels, contours= tf.train.shuffle_batch(
        [image, label, contour ],
        batch_size=batch_size,
        num_threads=num_preprocess_threads,
        capacity=min_queue_examples + 3 * batch_size,
    ...
    images, labels, contours = tf.train.batch(
        [image, label, contour ],
        batch_size=batch_size,
        num_threads=num_preprocess_threads,
        capacity=min_queue_examples + 3 * batch_size)
...
def inputs_train(mean_image,variance_image):
  ...
  contours_path = os.path.join(FLAGS.train_dir, 'Contours')

  image_names, label_names, contour_names = get_data_list(images_path, labels_path, contours_path, image_ext)
  ...
  if contours_path is not None:
    filename_queue = tf.train.slice_input_producer([image_names, label_names, contour_names],shuffle = True)
  else:
    filename_queue = tf.train.slice_input_producer([image_names, label_names],shuffle = True)

  # Read examples from files in the filename queue.
  image, label, contour = read_data(filename_queue)
  if contours_path is not None:
    image, label, contour  = process_image_and_label(image, label, contour , source_size, 
      target_size, ground_truth_size, mean_image,variance_image)
    # Generate a batch of images and labels by building up a queue of examples.
    return generate_image_label_contour_batch(image, label, contour , min_queue_examples, 
      FLAGS.train_batch_size, shuffle=False)
  else:
    image, label = process_image_and_label(image, label, contour , source_size, 
      target_size, ground_truth_size, mean_image,variance_image)
    # Generate a batch of images and labels by building up a queue of examples.
...
def inputs_val(mean_image,variance_image):
  ...
  contours_path = os.path.join(FLAGS.val_dir, 'Contours')
