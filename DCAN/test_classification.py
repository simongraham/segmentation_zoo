...
def testing(model_path, path_id, filename_list):
	...
	with tf.Graph().as_default():
		...
		with tf.variable_scope("network") as scope:
			if FLAGS.model_name == 'dcan':
				(softmax_train_gland1, softmax_train_gland2, softmax_train_gland3, logits_test, softmax_train_contour1, 
					softmax_train_contour2, softmax_train_contour3, logits_test_contour) = Segmentation_networks.dcan(images_test,keep_prob)
			else:
